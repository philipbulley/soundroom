...
export class Confirmation extends React.Component<Props, State> {
	...
	componentDidUpdate(prevProps: Props) {
		const viewState = this.getViewState(this.props.playlistCreate);
		const viewStatePrev =
			prevProps.playlistCreate && this.getViewState(prevProps.playlistCreate);
	...
	componentWillReceiveProps(nextProps: Props) {
		this.setState({
			...
			viewStatePrev:
				...
				this.getViewState(this.props.playlistCreate)
		});
	}
	...
	setSpinnerRef = ref => (this.spinner = ref);
	setTickRef = ref => (this.tick = ref);
	setCrossRef = ref => (this.cross = ref);
	setBackButtonRef = ref => (this.backButton = ref);
	...
	render() {
		...
		return (
			<ConfirmationStyled className="debug--confirmation-styled">
				<Layer innerRef={this.setSpinnerRef}>
					<Icon id="circle-o-notch" size={3} spin />
				</Layer>

				<Layer innerRef={this.setTickRef}>
					<Icon id="check" size={3} />
				</Layer>

				<Layer innerRef={this.setCrossRef} verticalOffset="-10%">
					<Icon id="close" size={3} />
				</Layer>

				<Layer innerRef={this.setBackButtonRef} verticalOffset="10%">
