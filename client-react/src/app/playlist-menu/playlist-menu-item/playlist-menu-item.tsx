...
class PlaylistMenuItem extends React.Component<PlaylistMenuItemProps> {
  ...
  doTransition = (node, done) => {
    ...
    if (inProp) {
      ...
      this.tl.add([
        TweenMax.fromTo(this.name, .8, {y: 30, opacity: 0}, {
          y: 0, opacity: 1,
          ease: Expo.easeOut
        }),
        TweenMax.fromTo(this.joinButton, .8, {y: 30, opacity: 0}, {
          y: 0, opacity: 1,
          ease: Expo.easeOut,
          onComplete: () => {
      ...
      this.tl.add(TweenMax.to(this.name, 0.5, {
        scale: 0.5,
        ease: Back.easeIn,
        onComplete: () => {
          console.log('About to call done():', playlist.name);
        ...
        <WrapperComponent>
          <Host className={className}>
            ...
            <Link className="button" to={'/room/' + playlist._id}>
              <Button innerRef={ref => this.joinButton = ref}>Join Room</Button>
            </Link>
          </Host>
        </WrapperComponent>
      </Transition>
